...
# Import the new chart data tool
from chart_data_tool import get_chart_data, get_database_chart_data
...
async def ask_stream(user_input: str):
    def generate_response():
        try:
            ...
            # Bind all tools including Jesse tools AND new chart data tools
            client_with_tools = client.bind_tools([
                ...
                get_crypto_raw_historical_data,
                get_chart_data,
                get_database_chart_data
            ])
            
            # Enhanced system prompt with chart data capabilities
...
5. Chart-ready data formatting (get_chart_data, get_database_chart_data)
...
CHART DATA CAPABILITIES:
When users ask for charts, graphs, or visual data:
- Use get_chart_data(symbol, chart_type, days) for formatted chart data
- Supported chart types: "price", "sentiment", "volume"
- The tool returns JSON data that can be directly used by the frontend chart renderer
- For specific date ranges, include start_date and end_date parameters

CHART USAGE EXAMPLES:
- "Show me BTC price chart for 30 days" → get_chart_data("BTC", "price", 30)
- "ETH sentiment chart last week" → get_chart_data("ETH", "sentiment", 7)  
- "Volume chart for SOL" → get_chart_data("SOL", "volume", 30)

When you return chart data:
1. Explain what the chart will show
2. Return the formatted JSON data
3. Tell the user it's ready for rendering
...
- Price history/trends/performance: Use get_crypto_historical_analysis (convert emoji format to UP/DOWN tags)
- Specific date ranges: Use get_crypto_historical_analysis_date_range
- Charts/graphs/visual data: Use get_chart_data or get_database_chart_data
- Raw data for analysis: Use get_crypto_raw_historical_data
            ...
            if result.tool_calls:
                for tool_call in result.tool_calls:
                    ...
                    tool_fn = {
                        ...
                        "get_database_chart_data": get_database_chart_data
                    }[tool_call["name"]]
...
CHART DATA CAPABILITIES:
- For charts/graphs: Use get_chart_data(symbol, chart_type, days)
- Supported chart types: "price", "sentiment", "volume"
- Returns JSON data ready for frontend rendering
...
# === New endpoint specifically for chart data ===
@app.get("/chart")
async def get_chart_endpoint(
    ...
    """
    Direct endpoint for getting chart data without LLM processing.
    """
    try:
        # Use the chart data tool directly
        if start_date and end_date:
            result = get_chart_data.invoke({
                "symbol": symbol,
